...
- 在选角界面切换当前角色的皮肤。
- 在卡牌总览中切换当前角色或颜色的卡牌皮肤；每个卡池单独保存，普通卡与该角色的远古卡图一起切换。
- 在怪物图鉴切换当前怪物的皮肤。
...
将发布目录放到游戏的 `mods/STS2SkinChanger` 下并正常启用。角色皮肤选项会出现在选角界面；卡牌皮肤选项会出现在卡牌总览“多人模式卡牌”选项上方，并跟随当前选中的角色或颜色卡池；怪物皮肤选项会出现在怪物图鉴；远古图鉴与角色数据位于图鉴底部的同一级入口。没有对应皮肤时不会显示切换控件。

仅替换皮肤资源、并正确声明 `affects_gameplay=false` 的 Mod 才会作为候选项出现。DLL 外观 Mod 会根据它提供的模板资源、启用配置或远古图片目录加入相应选项；混合玩法内容的 Mod 会作为当前基础资源的一部分，不会被误判为皮肤。
