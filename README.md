...
仅替换皮肤资源、并正确声明 `affects_gameplay=false` 的 Mod 才会作为候选项出现。DLL 外观 Mod 会根据它提供的模板资源、启用配置或远古图片目录加入相应选项；PCK 卡图会按卡牌总览的实际筛选规则归类，而不是只按角色文件夹判断。混合玩法内容的 Mod 会作为当前基础资源的一部分，不会被误判为皮肤。
