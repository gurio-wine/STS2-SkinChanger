...
| 顺序 | 级别 | 待办 | 排序理由 / 下一步 |
|---:|---|---|---|
| 1 | P1 | 返回选角界面无响应，只播放音乐 | 可能是界面生命周期阻断；先补问单人/多人、版本和日志。 |
| 2 | P1 | Save & Quit / Give Up 后 `FontVariation` 白屏卡死 | 有旧堆栈但无法复现；需要新版日志和完整 Mod 列表。 |
| 3 | P1 | 进入下一个房间闪退 | 高影响但信息最少；先收集版本、房间、角色、皮肤和日志。 |
| 4 | P2 | 异画卡被错误套用先古外形 | 系统性卡牌来源串用；检查单一优先级赢家是否同时控制卡图、外壳和类型。 |
| 5 | P2 | `Card Art Editor` 多卡面切换和悬浮预览失效 | 兼容性明确但不阻断对局；检查动态生成资源的注册与缓存失效。 |
| 6 | P2 | Nekobinder 预览碎片、Moe-Necrobinder 小手残留 | 放在同一轮检查角色骨骼、附件和退出回滚。 |
| 7 | P2 | 卡牌奖励界面卡顿 | 需要针对奖励界面做性能采样，不能用图鉴优化结果代替。 |
| 8 | P2 | 角色被拖出屏幕后无法恢复 | 增加不依赖目标点击的恢复入口或快捷键。 |
| 9 | P2 | 独立头像 Mod 无法与角色皮肤组合 | 需要把头像做成独立来源，而不是继续添加个别 Mod 特判。 |
| 10 | P2 | 亡灵契约师/猎手错位、劣人 TV 组合问题 | 前者缺复现信息；后者当前本机可用，优先等待原留言者复测。 |
| 11 | P3 | 商人局外入口、角色悬浮预览、按钮位置和入口开关 | 都是可用性增强，不应抢占崩溃和资源串用修复。 |
| 12 | P3 | 同 ID 差分 Mod 同时加载 | 涉及加载器身份和程序集冲突，成本高、风险大，最后评估。 |
