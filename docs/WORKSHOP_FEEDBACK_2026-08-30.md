...
| 08-29 05:58 | 加载顺序提醒 | 多次开关 Mod 后提醒不再弹出；另有玩家点击“置前并重启”时报错。 | **本地修复待发布**：日志确认报错玩家有 15 个皮肤提供者（含 `Merchant2CuteII`）排在 Skin Changer 前面。现在启用本 Mod 后会自动移到所有皮肤 Mod 之前，再弹出只含“重启”和“知道了”的提示；旧版“不再提示”设置不再阻止自动调整。 | [Bug 讨论](https://steamcommunity.com/workshop/filedetails/discussion/3787302680/591813130434318585/#c592939664046019968) |
...
| 顺序 | 级别 | 待办 | 排序理由 / 下一步 |
|---:|---|---|---|
| 1 | P1 | 发布并复测加载顺序提醒与跨平台重启 | 本地修复已完成；玩家日志证明错误加载顺序会同时造成 `Merchant2CuteII` 跑位和角色商店场景资源串用，发布后需确认自动置前和“重启/知道了”提示能稳定生效。 |
