...
| 08-31 本地复测 | CZN 系列 | 全 Mod 环境下，CZN 角色自己的选角表演方式和预览背景控件消失，卡图加载伴随大量资源错误；需要让 Skin Changer 的隔离流程保留 CZN 自身配置与 StyleUI 控制链。 | **已处理待复测**：已让动态选角控件与背景在提供者资源仍挂载时重建，原始卡图改为直接读取，并同步 CZN 怪物开关的读写状态。 | 本地复测 |

## 按紧急程度的处理顺序

| 顺序 | 级别 | 待办 | 排序理由 / 下一步 |
|---:|---|---|---|
| 1 | P1 | CZN 系列在全 Mod 环境下接管不完整 | 已修复并本地部署，等待全 Mod 环境复测选角控件、卡图和怪物开关；确认后删除此项。 |
