...
| 4 | P1 | 加载顺序反复误报 | 用反馈列出的五个 Mod 对照持久化顺序和运行时顺序，确认自动前移后的索引是否仍读取旧快照。 |
| 5 | P2 | 卡牌奖励、图鉴首次浏览与多人出牌卡顿 | 分别采样三个入口；只预热即将可见的当前皮肤，不扫描或实例化全部来源。 |
| 6 | P2 | 独立头像来源 | 设计为独立于角色整套皮肤的头像来源和优先级，并覆盖选角按钮、多人头像、左上角头像与地图图标。 |
| 7 | P2 | 角色拖出屏幕后无法恢复 | 增加无需点击目标即可恢复当前玩家模型变换的入口或快捷键。 |
| 8 | P3 | 局内入口开关、选角按钮位置、角色悬浮预览、“跟随分类”按钮位置 | 合并为一轮界面设置，不抢占崩溃、串皮、资源回滚和性能问题。 |
| 9 | P3 | 同 ID 差分包同时加载 | 涉及 Mod 身份、程序集和同路径资源冲突，成本与回归风险最高，最后单独设计。 |
