...
| 6 | P1 | Save & Quit / Give Up 后 `FontVariation` 白屏卡死 | 有旧堆栈但无法复现；需要新版日志和完整 Mod 列表。 |
| 7 | P1 | 进入下一个房间闪退 | 高影响但信息最少；先收集版本、房间、角色、皮肤和日志。 |
| 8 | P2 | 异画卡被错误套用先古外形 | 系统性卡牌来源串用；检查单一优先级赢家是否同时控制卡图、外壳和类型。 |
| 9 | P2 | `Card Art Editor` 多卡面切换和悬浮预览失效 | 兼容性明确但不阻断对局；检查动态生成资源的注册与缓存失效。 |
| 10 | P2 | Nekobinder 预览碎片、Moe-Necrobinder 小手残留 | 放在同一轮检查角色骨骼、附件和退出回滚。 |
| 11 | P2 | 卡牌奖励界面卡顿 | 需要针对奖励界面做性能采样，不能用图鉴优化结果代替。 |
| 12 | P2 | 选角切换卡顿 | 本地已有优化，先发布并复测，再决定是否继续细化。 |
| 13 | P2 | 角色被拖出屏幕后无法恢复 | 低成本但能避免永久错误配置；增加独立恢复入口或快捷键。 |
| 14 | P2 | 独立头像 Mod 无法与角色皮肤组合 | 需要把头像做成独立来源，而不是继续添加个别 Mod 特判。 |
| 15 | P2 | 亡灵契约师/猎手错位、劣人 TV 组合问题 | 前者缺复现信息；后者当前本机可用，优先等待原留言者复测。 |
| 16 | P3 | 局内切换卡牌皮肤后卡牌消失、无法抽牌 | 报告者和当前本机均未再复现；正式版静态与资源验证通过。暂不改运行逻辑，再现时收集当局日志和完整 Mod 列表。 |
