...
| 2 | P0 | “闪刀姬零衣”导致启动失败 | 启动阻断优先级最高；当前先等玩家复测，复现后核对自动排序前后的真实依赖和初始化顺序。 |
| 3 | P0 | 机器人皮肤进入战斗闪退、进入下个房间闪退 | 两项均缺关键材料；先拿到准确 Mod 名、游戏版本、复现房间和日志，再决定是否属于同一路径。 |
| 4 | P1 | 返回选角无响应与 `FontVariation` 白屏 | 都涉及界面返回和资源释放生命周期；先补齐新版日志，再检查是否能并入同一轮场景退出审计。 |
| 5 | P1 | 多人商人、遗物宝箱、营火场景串成房主皮肤 | 有明确场景和预期，且是多人按玩家隔离的缺口；优先写非战斗场景的双同角色回归用例。 |
| 6 | P1 | 选角热切换的骨骼、附件和特效层残留 | 使用 `3747942752`、阿洛娜、Nekobinder、Moe-Necrobinder 和 A 大骨妹组成最小复现组，验证双向切换而不是只测一种进入顺序。 |
| 7 | P1 | 卡牌差分控件被隔离 | 先以二次元猎人卡图的背包差分和蛇咬先古样式为样本，确认变体数据来源，再决定统一转成 Skin Changer 选项还是安全保留原控件。 |
| 8 | P1 | Card Art Editor 多卡面与“预览正常、应用跳变” | 先向最新留言者补问具体 Mod 和卡牌；随后统一检查预览临时选择、保存选择和最终资源赢家是否一致。 |
| 9 | P1 | 加载顺序反复误报 | 用反馈列出的五个 Mod 保存一次顺序后重启，记录 Skin Changer 判断到的索引和真实 Mod Loader 索引；避免自动排序反过来破坏依赖。 |
| 10 | P2 | 卡牌奖励界面卡顿 | 对奖励生成路径单独采样，不再用卡牌图鉴优化结果替代；确认是当前选中卡图加载还是全来源扫描。 |
| 11 | P2 | 独立头像来源 | 设计为独立于角色整套皮肤的头像来源和优先级，并覆盖选角按钮、多人头像、左上角头像与地图图标。 |
| 12 | P2 | 角色拖出屏幕后无法恢复 | 增加无需点击目标即可恢复当前玩家模型变换的入口或快捷键。 |
| 13 | P2 | 亡灵契约师/猎手错位、劣人 TV 组合 | 前者继续等具体 Mod 和日志；后者等原留言者复测，出现新证据再提升优先级。 |
| 14 | P3 | 局内入口开关、选角按钮位置、角色悬浮预览、“跟随分类”按钮位置 | 都是可用性增强，合并为一轮界面设置，不抢占崩溃、串皮和资源回滚问题。 |
| 15 | P3 | 同 ID 差分包同时加载 | 涉及 Mod 身份、程序集和同路径资源冲突，成本与回归风险最高，最后单独设计。 |
