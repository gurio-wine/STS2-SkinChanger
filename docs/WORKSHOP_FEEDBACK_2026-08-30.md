...
| 08-31 10:18 | 战斗崩溃 | 与一个机器人皮肤同时启用时，进入战斗直接闪退。 | **信息不足（高影响）**：原帖只给了第三方站点地址，没有可确认的 Mod 名、工坊 ID、游戏版本和日志。 | [旧问题讨论](https://steamcommunity.com/workshop/filedetails/discussion/3787302680/591813130434318585/) |
...
| 08-31 至 09-01 | 启动失败 | 与“闪刀姬零衣”同时启用时无法启动；把该 Mod 放在 Skin Changer 前面能启动，但自动调整顺序后再次无法启动。 | **已处理待复测**：本机后来无法复现，已经请反馈者用新版重测；若仍存在，需要日志和准确工坊条目。 | [新问题讨论](https://steamcommunity.com/workshop/filedetails/discussion/3787302680/592939955001664624/) |
| 09-01 07:48 | 黑屏 / 加载顺序 | 每次启动后的第二局会黑屏；同时每次启用 Skin Changer 都提示若干皮肤 Mod 排在它前面，但玩家称工坊和 Mod Loader 中这些 Mod 实际都在后面。涉及 `Watcher Beautified`、`RegentCardsAnimeRework`、`AncientWaifus_Beta`、`LieRenTVmod`、`Crimson Blade Valkyrie`。 | **信息不足（高影响）**：作者使用同组 Mod 未复现，已追问操作系统并索要黑屏日志。黑屏和顺序误判应分开验证，但可共用该玩家的环境材料。 | [留言](https://steamcommunity.com/sharedfiles/filedetails/?id=3787302680#comment_587310457621317137) |
...
| 2 | P0 | 第二局黑屏 | 单人或多人尚不明确，但属于会阻断继续游玩的稳定性问题；使用反馈者列出的五个 Mod 做双版本“首次对局 → 退出 → 第二局”复现。 |
| 3 | P0 | “闪刀姬零衣”导致启动失败 | 启动阻断优先级最高；当前先等玩家复测，复现后核对自动排序前后的真实依赖和初始化顺序。 |
| 4 | P0 | 机器人皮肤进入战斗闪退、进入下个房间闪退 | 两项均缺关键材料；先拿到准确 Mod 名、游戏版本、复现房间和日志，再决定是否属于同一路径。 |
| 5 | P1 | 返回选角无响应与 `FontVariation` 白屏 | 都涉及界面返回和资源释放生命周期；先补齐新版日志，再检查是否能并入同一轮场景退出审计。 |
| 6 | P1 | 多人商人、遗物宝箱、营火场景串成房主皮肤 | 有明确场景和预期，且是多人按玩家隔离的缺口；优先写非战斗场景的双同角色回归用例。 |
| 7 | P1 | 选角热切换的骨骼、附件和特效层残留 | 使用 `3747942752`、阿洛娜、Nekobinder、Moe-Necrobinder 和 A 大骨妹组成最小复现组，验证双向切换而不是只测一种进入顺序。 |
| 8 | P1 | 卡牌差分控件被隔离 | 先以二次元猎人卡图的背包差分和蛇咬先古样式为样本，确认变体数据来源，再决定统一转成 Skin Changer 选项还是安全保留原控件。 |
| 9 | P1 | Card Art Editor 多卡面与“预览正常、应用跳变” | 先向最新留言者补问具体 Mod 和卡牌；随后统一检查预览临时选择、保存选择和最终资源赢家是否一致。 |
| 10 | P1 | 加载顺序反复误报 | 用反馈列出的五个 Mod 保存一次顺序后重启，记录 Skin Changer 判断到的索引和真实 Mod Loader 索引；避免自动排序反过来破坏依赖。 |
| 11 | P2 | 卡牌奖励界面卡顿 | 对奖励生成路径单独采样，不再用卡牌图鉴优化结果替代；确认是当前选中卡图加载还是全来源扫描。 |
| 12 | P2 | 独立头像来源 | 设计为独立于角色整套皮肤的头像来源和优先级，并覆盖选角按钮、多人头像、左上角头像与地图图标。 |
| 13 | P2 | 角色拖出屏幕后无法恢复 | 增加无需点击目标即可恢复当前玩家模型变换的入口或快捷键。 |
| 14 | P2 | 亡灵契约师/猎手错位、劣人 TV 组合 | 前者继续等具体 Mod 和日志；后者等原留言者复测，出现新证据再提升优先级。 |
| 15 | P3 | 局内入口开关、选角按钮位置、角色悬浮预览、“跟随分类”按钮位置 | 都是可用性增强，合并为一轮界面设置，不抢占崩溃、串皮和资源回滚问题。 |
| 16 | P3 | 同 ID 差分包同时加载 | 涉及 Mod 身份、程序集和同路径资源冲突，成本与回归风险最高，最后单独设计。 |
